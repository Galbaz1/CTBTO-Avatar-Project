...
class ConversationTimingTracker {
  ...
  recordBackendReceived(sessionId: string, userMessage: string) {
    ...
    if (timing.userStoppedSpeaking) {
      const latency =
        timing.backendReceived.timestamp - timing.userStoppedSpeaking.timestamp;
      console.log(
        `⏱️ [${sessionId.slice(0, 8)}] SPEECH_TO_BACKEND: ${latency.toFixed(0)}ms`,
      );
    }
  }
  ...
  recordLLMFirstToken(sessionId: string) {
    ...
    if (timing.userStoppedSpeaking) {
      const latency =
        timing.llmFirstToken.timestamp - timing.userStoppedSpeaking.timestamp;
      console.log(
        `⏱️ [${sessionId.slice(0, 8)}] USER_TO_LLM: ${latency.toFixed(0)}ms`,
      );
    }
  }
  ...
  recordAvatarStartedSpeaking(sessionId: string) {
    ...
    if (timing.userStoppedSpeaking) {
      const totalLatency =
        timing.avatarStartedSpeaking.timestamp -
        timing.userStoppedSpeaking.timestamp;
      console.log(
        `⏱️ [${sessionId.slice(0, 8)}] 🎯 TOTAL_RESPONSE_TIME: ${totalLatency.toFixed(0)}ms`,
      );
    }
    ...
    if (timing.llmFirstToken) {
      const ttsLatency =
        timing.avatarStartedSpeaking.timestamp - timing.llmFirstToken.timestamp;
      console.log(
        `⏱️ [${sessionId.slice(0, 8)}] LLM_TO_SPEECH: ${ttsLatency.toFixed(0)}ms`,
      );
    }
  }
  ...
  recordCardDisplayed(sessionId: string, cardType: string) {
    ...
    if (timing.llmFirstToken) {
      const cardLatency =
        timing.cardDisplayed.timestamp - timing.llmFirstToken.timestamp;
      console.log(
        `⏱️ [${sessionId.slice(0, 8)}] CARD_RENDER_TIME: ${cardLatency.toFixed(0)}ms (${cardType})`,
      );
    }
  }
...
export const markBackendTiming = (sessionId: string, event: string) => {
  ...
  console.log(
    `⏱️ [${sessionId.slice(0, 8)}] BACKEND_${event.toUpperCase()}: ${timestamp.toFixed(0)}ms`,
  );
};
