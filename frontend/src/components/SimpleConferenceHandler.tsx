...
const conferenceLog = {
  responseCheck: (text: string) => {
    console.log(
      `%c🔍 Checking Rosa response for conference content:`,
  ...
  speakerDetected: (speakerName: string) => {
    console.log(
      `%c🎤 Speaker detected in response: ${speakerName}`,
    ...
    (text: string) => {
      ...
      if (text.includes("[ROSA_SPEAKER:")) {
        ...
        if (speakerMatch) {
          ...
          const speaker = conference_data.speakers.find(
            (s) => s.id === speakerId,
          );
      ...
      if (text.includes("[ROSA_SCHEDULE]")) {
        if (onScheduleUpdate) {
          conferenceLog.uiUpdate(
            "schedule (via metadata)",
            conference_data.sessions,
          );
          onScheduleUpdate({
      ...
      if (!hasUpdate) {
        ...
        if (
          lowerText.includes("dr. sarah chen") ||
          lowerText.includes("sarah chen")
        ) {
          const speaker = conference_data.speakers.find((s) =>
            s.name.toLowerCase().includes("sarah chen"),
          );
          if (speaker && onSpeakerUpdate) {
        ...
        if (
          lowerText.includes("prof. mikhail volkov") ||
          lowerText.includes("mikhail volkov")
        ) {
          const speaker = conference_data.speakers.find((s) =>
            s.name.toLowerCase().includes("mikhail volkov"),
          );
          if (speaker && onSpeakerUpdate) {
            conferenceLog.speakerDetected(
              "Prof. Mikhail Volkov (via text parsing)",
            );
            conferenceLog.uiUpdate("speaker", speaker);
            onSpeakerUpdate(speaker);
            hasUpdate = true;
        ...
        if (
          lowerText.includes("dr. amira hassan") ||
          lowerText.includes("amira hassan")
        ) {
          const speaker = conference_data.speakers.find((s) =>
            s.name.toLowerCase().includes("amira hassan"),
          );
          if (speaker && onSpeakerUpdate) {
            conferenceLog.speakerDetected(
              "Dr. Amira Hassan (via text parsing)",
            );
            conferenceLog.uiUpdate("speaker", speaker);
            onSpeakerUpdate(speaker);
            hasUpdate = true;
        ...
        if (
          lowerText.includes("today's sessions") ||
  ...
  useDailyEvent(
    ...
    useCallback(
      (event: any) => {
        ...
        if (
          data?.event_type === "conversation.respond" &&
        ...
        if (
          data?.event_type === "conversation.interrupted" &&
          data.properties?.text
        ) {
        ...
        if (
          ...
          console.log(
            "🔍 Found text in other event type:",
            data.event_type,
            data.properties.text,
          );
        }
