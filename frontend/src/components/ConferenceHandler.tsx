...
const conferenceLog = {
  ...
  success: (functionName: string, resultType: string, count?: number) => {
    ...
    console.log(
      `%c✅ ${functionName} success: ${resultType}${countText}`,
...
export const ConferenceHandler: React.FC<ConferenceHandlerProps> = ({
    ...
    if (data?.event_type === "conversation.function_result") {
      ...
      try {
        if (functionName === "get_speaker_info" && functionResult?.success) {
          ...
          functionName === "get_session_info" &&
          functionResult?.success
        ) {
          await handleSessionResult(functionResult, callId);
        } else if (
          functionName === "get_conference_schedule" &&
          functionResult?.success
        ) {
          ...
          conferenceLog.error(
            functionName,
            functionResult.message || "Unknown error",
          );
        }
      } catch (error) {
  ...
  const sendConferenceUpdate = async (
    updateType: string,
    data: any,
    callId?: string,
  ) => {
