...
export const FullScreenCardContainer: React.FC<FullScreenCardContainerProps> = ({
  ...
  if (currentCardHash !== lastCardHash.current) {
    ...
    console.log(`🔧 [DEBUG] Cards data:`, cards.map(c => ({
      id: c.id, 
      type: c.type, 
  ...
  const { layoutVariant, activeCards, hasHeroCard, gridConfig } = useCardLayout(cards, maxCards);
        ...
        <AnimatePresence mode="popLayout">
          {activeCards.map((card, index) => {
            ...
            if (!CardComponent) {
              ...
              return (
                <div key={card.id} className="p-4 border border-red-300 bg-red-50 rounded-lg">
            ...
            return (
              ...
              >
                <CardComponent
                  ...
                  {...(card.data || {})}
                  onClose={card.type === 'weather' ? onCloseWeather : onCloseRag}
                  className="w-full"
                />
              </motion.div>
            );
