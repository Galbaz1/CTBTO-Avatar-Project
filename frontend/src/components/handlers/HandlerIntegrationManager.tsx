 ...
 */
export const HandlerIntegrationManager: React.FC<
  HandlerIntegrationManagerProps
> = ({
  meetingState,
  conversationId,
  onCardUpdate,
  ...
  const handleCardUpdate = useCallback(
    (cardData: any, cardType: string) => {
      try {
        ...
        const lastData =
          lastDataRefs.current[cardType as keyof typeof lastDataRefs.current];
        ...
        console.log(
          `📊 Handler Manager: ${cardType} card updated (Total: ${stats.totalUpdates + 1})`,
        );
      } catch (error) {
  ...
  const handleVenueUpdate = useCallback(
    (data: VenueCardData) => {
